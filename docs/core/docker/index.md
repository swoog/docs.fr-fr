---
title: Docker et .NET Core
description: Découvrez plus d’informations sur l’utilisation de Docker avec .NET Core grâce à ces didacticiels amusants.
author: jralexander
ms.author: johalex
ms.date: 01/10/2018
ms.openlocfilehash: e31a3a221604e7aca7d35a4826e78ade0f980a6b
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: HT
ms.contentlocale: fr-FR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33219295"
---
# <a name="docker-and-net-core"></a>Docker et .NET Core 

Les didacticiels suivants sont disponibles pour en savoir plus sur l’utilisation de Docker avec .NET Core.

- [Introduction à .NET et à Docker](intro-net-docker.md)
- [Découvrir les concepts de base de Docker avec .NET Core](docker-basics-dotnet-core.md)
- [Création d’images Docker pour les applications .NET Core](building-net-docker-images.md)
- [Visual Studio Tools pour Docker](/aspnet/core/host-and-deploy/docker/visual-studio-tools-for-docker)

Pour obtenir des didacticiels sur le développement d’applications web ASP.NET Core, consultez la [Documentation ASP.NET Core](/aspnet/core/).
