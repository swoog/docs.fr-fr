...
Les classes représentent la description fondamentale des types d’objet .NET ; la classe est le concept de type principal qui prend en charge la programmation orientée objet dans F#.
...
Le constructeur est un code qui crée une instance du type classe. Constructeurs des classes fonctionnent un peu différemment F# dans d’autres langages .NET. Dans un F# de classe, il y a toujours un constructeur principal dont les arguments sont décrits dans le `parameter-list` qui suit le nom de type, et dont le corps se compose de la `let` (et `let rec`) liaisons au début de la déclaration de classe et le `do` liaisons qui suivent. Les arguments du constructeur principal sont dans la portée dans l’ensemble de la déclaration de classe.
...
Le `inherit` clause identifie la classe de base directe, le cas échéant. Dans F#, un seul diriger la classe de base est autorisée. Les interfaces qu’une classe implémente ne sont pas considérés comme des classes de base. Interfaces sont traitées dans le [Interfaces](Interfaces.md) rubrique.
