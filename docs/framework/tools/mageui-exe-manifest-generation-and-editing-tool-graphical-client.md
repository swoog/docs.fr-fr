...
|-------------|----------|--------------|-----------------|  
|**Manifeste de l’application**|**Fichier, Nouveau**||Crée un nouveau manifeste d'application.|  
...
|**À propos de**|**Help**||Affiche la version et les informations de copyright concernant MageUI.exe.|  
...
|**Inclure l’emplacement de démarrage (ProviderURL) dans le manifeste**|Optionnel. Spécifie l'URL examinée par [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] pour les mises à jour d'application.|  
...
|**Autoriser la transmission des paramètres d’URL à l’application**|Obligatoire. Autorise le transfert de données de paramètre à l'application [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] via une chaîne de requête ajoutée à l'URL du manifeste de déploiement. Par défaut, la case est décochée.|  
...
|**Fréquence de vérification des mises à jour**|Détermine la fréquence à laquelle [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] doit vérifier les mises à jour :<br /><br /> -   **Vérifier à chaque exécution de l’application**. [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] effectue une vérification des mises à jour chaque fois que l’utilisateur ouvre l’application.<br />-   **Vérifier après** : Sélectionnez un intervalle de temps et une unité (heures, jours ou semaines) avant la vérification des mises à jour.|  
...
- [Procédure pas à pas : Déployer manuellement une application ClickOnce](/visualstudio/deployment/walkthrough-manually-deploying-a-clickonce-application)
