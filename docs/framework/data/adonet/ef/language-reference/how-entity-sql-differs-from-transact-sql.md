...
-   `in` et `exists` sous-requêtes ont été généralisées pour autoriser toute collection.  
  
     Une sous-requête est un type de collection. `e1 in e2` et `exists(e)` sont le [!INCLUDE[esql](../../../../../../includes/esql-md.md)] constructions pour effectuer ces opérations.  
...
### <a name="avoiding-implicit-coercions-for-subqueries"></a>Éviter des contraintes implicites pour les sous-requêtes  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne prend pas en charge cette contrainte implicite. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] fournit l’opérateur ANYELEMENT pour extraire une valeur singleton d’une collection et un `select value` clause pour éviter de créer un wrapper de ligne pendant une expression de requête.  
...
## <a name="select-value-avoiding-the-implicit-row-wrapper"></a>Select value : éviter le wrapper de ligne implicite  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] fournit le `select value` clause pour ignorer la construction de ligne implicite. Un seul élément peut être spécifié dans une clause `select value`. Lorsqu’une telle clause est utilisée, aucun wrapper de ligne n’est construit autour des éléments de la clause `select` et une collection de la forme souhaitée peut être générée, par exemple : `select value a`.  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] fournit également le constructeur row pour construire des lignes arbitraires. `select` prend un ou plusieurs éléments dans la projection et produit un enregistrement de données avec des champs, comme suit :  
...
## <a name="referencing-columns-properties-of-tables-collections"></a>Référencement de colonnes (propriétés) de tables (collections)  
 Toutes les références de colonne dans [!INCLUDE[esql](../../../../../../includes/esql-md.md)] doivent être qualifiées avec l'alias de la table. La construction suivante (en supposant que `a` est une colonne de table valide `T`) est valide dans [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] , mais pas dans [!INCLUDE[esql](../../../../../../includes/esql-md.md)].  
...
```  
 ...
 Les alias de la table sont facultatifs dans la clause `from`. Le nom de la table est utilisé comme alias implicite. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] permet également la forme suivante :  
...
## <a name="navigation-through-objects"></a>Navigation dans les objets  
 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] utilise le «. » pour référencer les colonnes de (une ligne de) une table. [!INCLUDE[esql](../../../../../../includes/esql-md.md)] étend cette notation (empruntée des langages de programmation) pour prendre en charge la navigation dans les propriétés d’un objet.  
...
## <a name="no-support-for-"></a>Aucune prise en charge pour *  
 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] prend en charge de la non qualifiée * syntaxe en tant qu’alias pour la ligne entière et qualifié \* syntaxe (t.\*) comme raccourci pour les champs de la table. En outre, [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] permettant à un compte spécial (\*) agrégat, qui inclut les valeurs NULL.  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne prend pas en charge le * construction. [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] requêtes de la forme `select * from T` et `select T1.* from T1, T2...` peuvent être exprimées dans [!INCLUDE[esql](../../../../../../includes/esql-md.md)] comme `select value t from T as t` et `select value t1 from T1 as t1, T2 as t2...`, respectivement. En outre, ces constructions gèrent l'héritage (capacité des valeurs à être substituées), tandis que les variantes `select *` sont restreintes aux propriétés de niveau supérieur du type déclaré.  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne prend pas en charge la `count(*)` agrégation. Utilisez plutôt `count(0)`.  
  
## <a name="changes-to-group-by"></a>Modifications apportées à Group By  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] prend en charge les alias des `group by` clés. Les expressions dans la clause `select` et la clause `having` doivent faire référence aux clés `group by` par le biais de ces alias. Par exemple, la syntaxe [!INCLUDE[esql](../../../../../../includes/esql-md.md)] :  
...
## <a name="order-by-clause-usage"></a>Utilisation des clauses ORDER BY  
 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] permet des clauses ORDER BY doit être spécifié uniquement dans le bloc SELECT... FROM . WHERE le plus élevé. Dans [!INCLUDE[esql](../../../../../../includes/esql-md.md)], vous pouvez utiliser une expression ORDER BY imbriquée et elle peut être placée n'importe où dans la requête, mais le classement dans une requête imbriquée n'est pas conservé.  
...
## <a name="transact-sql-functionality-not-available-in-entity-sql"></a>Fonctionnalités Transact-SQL non disponibles dans Entity SQL  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne fournit aucun support pour la programmation impérative, contrairement à [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]. Utilisez un langage de programmation à la place.  
  
 Fonctions de regroupement  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne fournit pas encore prise en charge pour le regroupement des fonctions (par exemple, CUBE, ROLLUP et GROUPING_SET).  
  
 Fonctions analytiques  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] pas (encore) prennent en charge pour les fonctions analytiques.  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] ne prend pas en charge le traitement par lot de résultats de requête. Par exemple, ce qui suit est valide [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] (envoi en tant que lot) :  
...
```  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] prend uniquement en charge une seule instruction de requête produire de résultat par commande.  
