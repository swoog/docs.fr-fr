...
L’exemple de code suivant fusionne deux `DataSet` objets avec des schémas différents en un seul `DataSet` avec les schémas combinées des deux entrant `DataSet` objets.
