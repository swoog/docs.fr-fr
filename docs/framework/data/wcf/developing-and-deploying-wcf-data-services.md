...
2. **Serveur de développement Visual Studio**
     ...
     Visual Studio inclut un serveur Web intégré, le serveur de développement Visual Studio, qui est le serveur Web par défaut pour les projets ASP.NET. Ce serveur Web est conçu pour exécuter des projets ASP.NET sur l’ordinateur local pendant le développement. Le [démarrage rapide WCF Data Services](../../../../docs/framework/data/wcf/quickstart-wcf-data-services.md) montre comment créer un service de données qui s’exécute dans le serveur de développement Visual Studio.
...
- **Serveur Web hébergé par IIS**
    ...
    Quand un service de données est développé en tant qu’un projet ASP.NET, il peut être déployé sur un serveur Web IIS à l’aide du processus de déploiement ASP.NET standards.  Visual Studio fournit les technologies de déploiement suivantes pour ASP.NET, en fonction du type de projet ASP.NET qui héberge le service de données que vous déployez.
