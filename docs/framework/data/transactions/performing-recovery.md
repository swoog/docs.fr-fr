...
### <a name="step-2---completing-the-recovery"></a>Étape 2 : Fin de la récupération  
 ...
 Il n'est pas nécessaire que le gestionnaire de ressources ait résolu toutes les transactions incertaines avant de procéder aux inscriptions à de nouvelles transactions. La première étape peut être effectuée à tout moment après que le Gestionnaire de ressources établit une relation avec le Gestionnaire de transactions, mais après <xref:System.Transactions.TransactionManager.RecoveryComplete%2A> a été appelée (étape 2) ; étape 1 ne peut pas être effectuée à nouveau. L'étape 2 peut être répétée plusieurs fois sans affecter le résultat des transactions.
