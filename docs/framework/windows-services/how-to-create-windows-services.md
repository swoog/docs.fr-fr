...
3. Définissez le fonctionnement de votre service à l'aide des propriétés suivantes.  
    ...
    |Property|Paramètre|  
    |--------------|-------------|  
    |<xref:System.ServiceProcess.ServiceBase.CanStop%2A>|`True` pour indiquer que le service acceptera les demandes d’arrêt ; `false` pour empêcher tout arrêt du service.|  
    ...
    |<xref:System.ServiceProcess.ServiceBase.CanPauseAndContinue%2A>|`True` pour indiquer que le service acceptera les demandes de suspension ou de reprise ; `false` pour empêcher toute suspension et toute reprise du service.|  
    |<xref:System.ServiceProcess.ServiceBase.CanHandlePowerEvent%2A>|`True` pour indiquer que le service peut gérer les notifications concernant les variations de l’état d’alimentation de l’ordinateur ; `false` pour empêcher le service d’être informé de ces changements.|  
...
- [Procédure : écrire les services par programmation](../../../docs/framework/windows-services/how-to-write-services-programmatically.md)
...
- [Procédure : enregistrer des informations relatives aux services](../../../docs/framework/windows-services/how-to-log-information-about-services.md)
