...
|`Switch.System.Drawing.`<br/>`DontSupportPngFramesInIcons`|Contrôles si le <xref:System.Drawing.Icon.ToBitmap%2A?displayProperty=nameWithType> méthode lève une exception quand un <xref:System.Drawing.Icon> objet comporte des cadres PNG. Pour plus d’informations, consultez [atténuation : Des cadres PNG dans les objets Icon](~/docs/framework/migration-guide/mitigation-png-frames-in-icon-objects.md).|.NET Framework 4.6|  
