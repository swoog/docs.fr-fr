...
> [!NOTE]
> Pour optimiser la productivité, le Concepteur de formulaires Windows dans Visual Studio shadows le <xref:System.Windows.Forms.Control.AutoSize%2A> propriété pour la <xref:System.Windows.Forms.Form> classe. Au moment du design, le formulaire se comporte comme si le <xref:System.Windows.Forms.Control.AutoSize%2A> propriété est définie sur `false`, quelle que soit sa valeur réelle. Lors de l’exécution, aucun aménagement spécial n’est effectuée et le <xref:System.Windows.Forms.Control.AutoSize%2A> propriété est appliquée comme spécifié par le paramètre de propriété.
