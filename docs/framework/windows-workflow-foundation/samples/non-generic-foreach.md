...
[!INCLUDE[netfx_current_long](../../../../includes/netfx-current-long-md.md)] est fourni dans sa boîte à outils, un ensemble d’activités de flux de contrôle, y compris <xref:System.Activities.Statements.ForEach%601>, ce qui permet l’itération <xref:System.Collections.Generic.IEnumerable%601> collections.  
...
|---------------|-------------|--------------|--------------------|  
|Est de valeurs `null`|La valeur d’un argument d’activité ’Values’ requis n’a pas été fournie.|Error|<xref:System.InvalidOperationException>|  
