...
<xref:System.Xaml.XamlServices> est une classe fournie par les Services XAML .NET Framework qui peut être utilisé pour des scénarios XAML ne nécessitant pas d’accès spécifique dans le flux de nœud XAML, ou les informations de système de type XAML obtenues à partir de ces nœuds. <xref:System.Xaml.XamlServices> API peut être résumée comme suit : `Load` ou `Parse` pour prendre en charge un chemin de chargement XAML, `Save` pour prendre en charge un XAML enregistrer le chemin d’accès, et `Transform` pour fournir une technique qui associe un chemin de chargement et de chemin d’enregistrement. `Transform` permet de modifier à partir d’un schéma XAML à un autre. Cette rubrique résume chacune de ces classifications d'API et décrit les différences qui existent entre des surcharges de méthode particulières.  
...
## <a name="parse"></a>Parse  
 <xref:System.Xaml.XamlServices.Parse%2A> est similaire à `Load` , car il est une API qui crée un flux de nœud XAML à partir de l’entrée XAML de chemin de chargement. Toutefois, dans ce cas, l'entrée XAML est fournie directement sous la forme d'une chaîne qui contient tout le code XAML à charger. <xref:System.Xaml.XamlServices.Parse%2A> est une approche légère plus appropriée pour les scénarios d’application les scénarios d’infrastructure. Pour plus d'informations, consultez <xref:System.Xaml.XamlServices.Parse%2A>. <xref:System.Xaml.XamlServices.Parse%2A> est simplement encapsulée <xref:System.Xaml.XamlServices.Load%28System.Xml.XmlReader%29> appel implique un <xref:System.IO.StringReader> en interne.  
