---
title: 226 - IdleServicesClosed
ms.date: 03/30/2017
ms.assetid: b5e72de3-33c6-46a2-998a-3809b520bb30
ms.openlocfilehash: d41cf1b4d4bfd59bfac99afa9a2fe5d5ebbaffac
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33458196"
---
# <a name="226---idleservicesclosed"></a>226 - IdleServicesClosed
## <a name="properties"></a>Propriétés  
  
|||  
|-|-|  
|ID|226|  
|Mots clés|HealthMonitoring, WebHost|  
|Niveau|LogAlways|  
|Canal|Microsoft-Windows-Application Server-Applications/Analyse|  
  
## <a name="description"></a>Description  
 Cet événement est émis lorsque les services inactifs sont fermés.  
  
## <a name="message"></a>Message  
 %1 services inactifs sur un total de %2 services activés ont été fermés.  
  
## <a name="details"></a>Détails
