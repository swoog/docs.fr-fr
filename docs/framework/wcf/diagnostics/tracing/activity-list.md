...
Cette rubrique répertorie toutes les activités définies par le service Windows Communication Foundation (WCF).  
  
> [!NOTE]
>  Vous pouvez également définir des activités par programme pour grouper des suivis utilisateur. Pour plus d’informations, consultez [générant des Traces de Code utilisateur](../../../../../docs/framework/wcf/diagnostics/tracing/emitting-user-code-traces.md).  
...
|-----------|-------------------|-------------------|-----------------|  
|A, M|Activité ambiante|N/A (non contrôlé par ServiceModel)|Activité dont l'ID est défini dans TLS avant les appels au code ServiceModel (côté client ou côté serveur).<br /><br /> Exemple : Une activité où l’ouverture est appelée sur le client WCF ou serviceHost.open est appelée.|  
...
|Nom de l'activité|Type d'activité|Description|  
|-------------------|-------------------|-----------------|  
|Créer une instance COM+|TransferToCOMPlus|1 instance d’activité pour chaque COM + appeler à partir de code WCF|  
|Exécuter COM + \<opération >|TransferToCOMPlus|1 instance d’activité pour chaque COM + appeler à partir de code WCF|  
