...
WCF prend désormais en charge le développement Contrat en premier. L’outil svcutil.exe a un commutateur /serviceContract qui vous permet de générer des contrats de service et des données à partir d’un document WSDL.
...
> [!IMPORTANT]
> Reportez-vous à « À l’aide de XML en toute sécurité » sous [considérations de sécurité pour les données](../../../docs/framework/wcf/feature-details/security-considerations-for-data.md) pour plus d’informations sur la sécurisation de vos données.
