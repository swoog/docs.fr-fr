...
- Modifications du contrat : Par exemple, une opération peut être ajoutée, ou un élément de données dans un message peut être ajouté ou modifié.  
  
- Modifications d’adresse : Par exemple, un service se déplace vers un autre emplacement où les points de terminaison ont les nouvelles adresses.  
  
- Modifications de liaison : Par exemple, un mécanisme de sécurité change ou modifier ses paramètres.  
...
### <a name="lax-versioning"></a>Contrôle de version souple  
 ...
 Pour obtenir des instructions détaillées pour le contrôle de version strict et souple des contrats de données, consultez [meilleures pratiques : Le contrôle de version de contrat de données](../../../docs/framework/wcf/best-practices-data-contract-versioning.md).  
...
## <a name="message-contract-versioning"></a>Contrôle de version des contrats de message  
 ...
 Des en-têtes de message peuvent toujours être ajoutés, même si le contrôle de version strict est utilisé. L'indicateur MustUnderstand peut affecter le contrôle de version. En règle générale, le modèle de contrôle de version pour les en-têtes dans WCF est comme décrit dans la spécification SOAP.  
