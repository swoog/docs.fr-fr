...
```  
 ...
 Le `<in-reply-to>` élément spécifie trois attributs requis (`ref`, `type` et `href`) tout en autorisant également la présence d’attributs d’extension supplémentaires et des éléments d’extension.  
