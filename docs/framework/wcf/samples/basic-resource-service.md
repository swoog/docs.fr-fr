...
## <a name="sample-details"></a>Détails de l'exemple  
 ...
 Le fichier App.config configure le service WCF avec un <xref:System.ServiceModel.Description.WebHttpEndpoint> par défaut dont la propriété <xref:System.ServiceModel.Description.WebHttpEndpoint.HelpEnabled%2A> a la valeur `true`. Par conséquent, WCF crée une page d’aide HTML automatique sous `http://localhost:8000/Customers/help` qui fournit des informations sur la construction HTTP requêtes adressées au service et comment accéder à la réponse du service HTTP, par exemple, un exemple de la façon dont les détails du client est représenté dans JSON et XML.  
