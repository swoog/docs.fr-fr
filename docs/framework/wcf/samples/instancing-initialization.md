...
- <xref:System.ServiceModel.Dispatcher.IInstanceProvider.ReleaseInstance%2A>: Lorsque l’instance de service a terminé le traitement du message, l’EndpointDispatcher appelle la <xref:System.ServiceModel.Dispatcher.IInstanceProvider.ReleaseInstance%2A> (méthode). À l'instar de la méthode <xref:System.ServiceModel.Dispatcher.IInstanceProvider.GetInstance%2A>, la fréquence des appels à cette méthode est déterminée par la propriété <xref:System.ServiceModel.ServiceBehaviorAttribute.InstanceContextMode%2A>.  
...
- Comportements de point de terminaison : Permettent la personnalisation d’un point de terminaison de service particulier, y compris l’EndpointDispatcher.  
  
- Comportements de contrat : Permettent de personnaliser la <xref:System.ServiceModel.Dispatcher.ClientRuntime> ou <xref:System.ServiceModel.Dispatcher.DispatchRuntime> classes sur le client ou le service respectivement.  
  
- Comportements d’opération : Permettent de personnaliser la <xref:System.ServiceModel.Dispatcher.ClientOperation> ou <xref:System.ServiceModel.Dispatcher.DispatchOperation> classes sur le client ou le service respectivement.  
