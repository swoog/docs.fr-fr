...
ms.locfileid: "33494860"
---
# <a name="choosing-a-message-encoder"></a>Sélection d'un encodeur de message
Cette rubrique traite des critères permettant de choisir parmi les encodeurs de message sont inclus dans Windows Communication Foundation (WCF) : binaire, texte et Message Transmission Optimization Mechanism (MTOM).  
...
## <a name="system-provided-encoders"></a>Encodeurs fournis par le système  
 WCF inclut trois encodeurs de message sont représentées par les trois classes suivantes :  
...
-   <xref:System.ServiceModel.Channels.BinaryMessageEncodingBindingElement>, l’encodeur de message binaire utilise un format binaire compact et est optimisé pour WCF pour la communication WCF et n’est donc pas interopérable. Il s’agit également l’encodeur performant la plupart de tous les encodeurs que WCF fournit.  
  
-   <<!--zz xref:System.ServiceModel.Channels.MTOMMessageEncodingBindingElement --> `System.ServiceModel.Channels.MTOMMessageEncodingBindingElement`>, l’élément de liaison spécifie l’encodage de caractères et le versioning de messages pour les messages encodés avec MTOM. MTOM est une technologie efficace de transmission de données binaires dans les messages WCF. L'encodeur MTOM tente de créer un équilibre entre rendement et interopérabilité. L'encodage MTOM transmet la plupart du XML sous forme textuelle, mais optimise les grands blocs de données binaires en les transmettant tels quels, sans conversion en texte. En termes de rendement, parmi les encodeurs que WCF fournit, MTOM est texte intermédiaires (la plus lente) et binary (le plus rapide).  
