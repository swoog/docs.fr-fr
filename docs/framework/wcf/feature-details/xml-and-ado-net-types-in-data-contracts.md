...
```  
 ...
 Lorsqu'ils sont utilisés avec `DataContractSerializer`, les tableaux `XmlNode` peuvent être assignés d'une manière polymorphe mais uniquement à un membre de données de type `Object`. Même s'il implémente `IEnumerable`, un tableau de `XmlNode` ne peut pas être utilisé comme un type de collection et ne peut pas être assigné à un membre de données `IEnumerable`. Comme avec toutes les assignations polymorphes, le `DataContractSerializer` émet le nom de contrat de données dans le XML résultant – dans ce cas, il est « ArrayOfXmlNode » dans le «http://schemas.datacontract.org/2004/07/System.Xml« espace de noms. Lorsqu’il est utilisé avec le `NetDataContractSerializer`, les assignations valides d’un `XmlNode` tableau est pris en charge.  
