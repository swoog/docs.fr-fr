...
|Entrant, provenant de la pile de canaux de diffusion en continu|Objet `Stream` qui représente les données entrant sur le réseau avec un <xref:System.Xml.XmlReader> placé sur celui-ci|Écrire le contenu à partir de la commande stockée `XmlReader` à l’aide de `WriteNode`|Retourne les informations stockées `XmlReader`|  
|Entrant, provenant de la pile de canaux sans diffusion en continu|Mémoire tampon qui contient les données relatives au corps avec un `XmlReader` placé sur celle-ci|Écrit le contenu à partir de la liste stockée `XmlReader` à l’aide de `WriteNode`|Retourne le langage stocké|  
...
### <a name="transport-channels-and-message-encoders"></a>Canaux de transport et encodeurs de message  
 ...
 La plupart des canaux de transport utilisent des composants appelés *encodeurs de message* pour procéder à l'encodage et au décodage. Un encodeur de message est une sous-classe de la classe <xref:System.ServiceModel.Channels.MessageEncoder> . `MessageEncoder` inclut différentes `ReadMessage` et `WriteMessage` des surcharges de méthode pour convertir entre `Message` et des groupes d’octets.  
