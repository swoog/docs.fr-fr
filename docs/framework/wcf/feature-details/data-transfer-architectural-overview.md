...
## <a name="message-properties"></a>Propriétés de message  
 Un message peut contenir des propriétés. Un *propriété* est n’importe quel objet .NET Framework qui est associé à un nom de chaîne. Les propriétés sont accessibles via la propriété `Properties` sur `Message`.  
