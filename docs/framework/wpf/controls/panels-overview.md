...
#### <a name="virtualizingstackpanel"></a>Élément VirtualizingStackPanel  
 ...
 L’exemple suivant montre comment utiliser un <xref:System.Windows.Controls.VirtualizingStackPanel> comme un hôte d’éléments. Le <xref:System.Windows.Controls.VirtualizingStackPanel.IsVirtualizingProperty?displayProperty=nameWithType> propriété jointe doit être définie pour `true` (par défaut) pour la virtualisation de se produire.  
