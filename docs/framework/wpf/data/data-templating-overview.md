...
## <a name="prerequisites"></a>Prérequis  
 ...
 <xref:System.Windows.DataTemplate> concerne la présentation des données et est une des nombreuses fonctionnalités fournies par le modèle de styles et modèles WPF. Pour obtenir une présentation du WPF styles et modèles modèle, notamment comment utiliser un <xref:System.Windows.Style> pour définir des propriétés sur les contrôles, consultez le [Styling and Templating](../../../../docs/framework/wpf/controls/styling-and-templating.md) rubrique.  
  
 En outre, il est important de comprendre `Resources`, qui permettent essentiellement objets tels que <xref:System.Windows.Style> et <xref:System.Windows.DataTemplate> pour être réutilisables. Pour plus d’informations sur les ressources, consultez la page [Ressources XAML](../../../../docs/framework/wpf/advanced/xaml-resources.md).  
