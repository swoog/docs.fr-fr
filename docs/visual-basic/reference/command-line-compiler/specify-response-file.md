...
```  
 ...
 Visual Basic fournit le fichier Vbc.rsp, qui se trouve dans le même répertoire que le fichier Vbc.exe. Le fichier Vbc.rsp est inclus par défaut, sauf si la `-noconfig` option est utilisée. Pour plus d’informations, consultez [- /noconfig](../../../visual-basic/reference/command-line-compiler/noconfig.md).  
