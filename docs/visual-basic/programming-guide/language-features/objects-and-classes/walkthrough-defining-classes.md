...
2.  Dans la liste des modèles de projet Visual Basic pour afficher le nouveau projet, sélectionnez Application Windows.  
...
5.  Nommez la nouvelle classe `UserNameInfo.vb`, puis cliquez sur **ajouter** pour afficher le code pour la nouvelle classe.  
    ...
    > [!NOTE]
    >  Vous pouvez utiliser Visual Basic **éditeur de Code** pour ajouter une classe à votre formulaire de démarrage en tapant le `Class` (mot clé), suivi du nom de la nouvelle classe. Le **éditeur de Code** fournit correspondante `End Class` instruction pour vous.  
