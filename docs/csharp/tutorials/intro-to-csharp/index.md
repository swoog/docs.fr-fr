...
Le tutoriel [Interpolation de chaîne](interpolated-strings.yml) vous montre comment insérer des valeurs dans une chaîne. Vous allez apprendre à créer une chaîne interpolée avec des expressions C# incorporées et à contrôler la mise en forme de la chaîne de sortie. Vous pouvez également [exécuter ce tutoriel localement sur votre ordinateur](interpolated-strings-local.md).

Ce tutoriel part du principe que vous avez fini les leçons [Hello world](hello-world.yml), [Nombres en C#](numbers-in-csharp.yml) et [Branches et boucles](branches-and-loops.yml).
