...
L’opérateur de reste `%` calcule le reste après avoir divisé son premier opérande par son second opérande. Les types définis par l’utilisateur peuvent [surcharger](../keywords/operator.md) l’opérateur `%`. Quand `%` est surchargé, l’[opérateur d’assignation de reste](remainder-assignment-operator.md) `%=` est également surchargé implicitement.
