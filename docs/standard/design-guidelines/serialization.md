 ...
 **X AVOID** prenant en charge la sérialisation du Runtime ou la sérialisation XML uniquement pour des raisons de persistance générales. Préférez la sérialisation de contrat de données à la place.  
 ...
 **✓ DO** s’appliquent à une demande de liaison <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A?displayProperty=nameWithType> implémentation. Cela garantit qu’approuvé uniquement entièrement core et le sérialiseur de Runtime ont accès au membre.  
