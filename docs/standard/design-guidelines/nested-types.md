 ...
 **X AVOID** exposées publiquement des types imbriqués. La seule exception est si les variables du type imbriqué doivent être déclarées que dans les rares scénarios sous-classement ou d’autres scénarios de personnalisation avancées.  
 ...
 **X DO NOT** utilisez des types imbriqués s’ils doivent être instancié par le code client.  Si un type a un constructeur public, il doit probablement pas être imbriqué.  
 ...
 **X DO NOT** définir un type imbriqué en tant que membre d’une interface. De nombreux langages ne gèrent pas une construction de ce type.  
