...
```  
 ...
 Lorsqu'une variable est liée à un fragment d'arborescence résultat au lieu d'un des quatre types de données XPath de base, c'est la seule fois où une requête XPath retourne un type qui ne correspond pas à un des quatre types d'objet XPath. Les fragments d’arborescence résultat et leur comportement sont expliqués dans la spécification du World Wide Web Consortium (W3C) à l’adresse www.w3.org/XSLT, de la section 11.1 sur les fragments d’arborescence résultat à la section 11.6 sur le transfert des paramètres aux modèles. Également, la section 1 Introduction explique comment des modèles peuvent contenir des éléments provenant de l'espace de noms XSLT qui retournent ou créent des fragments d'arborescence résultat.  
