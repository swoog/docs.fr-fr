...
À l’aide de .NET appliquée autorisations et autres dans votre code, vous devez ériger des barrières pour empêcher un code malveillant d’accéder aux informations que vous souhaitez qu’il ait ou effectuer d’autres actions indésirables. En outre, il vous faut trouver un compromis entre la sécurité et les possibilités d’utilisation dans tous les scénarios prévus qui utilisent du code de confiance.
...
## <a name="managed-wrapper-to-native-code-implementation"></a>Wrapper pour l’implémentation de code natif managé

Dans ce scénario, certaines fonctionnalités utiles sont implémentées dans du code natif que vous souhaitez mettre à la disposition du code managé. Il est aisé d’écrire des wrappers managés en utilisant l’appel de code non managé ou COM Interop. Toutefois, si vous procédez ainsi, les appelants de vos wrappers doivent avoir des droits de code non managé pour réussir. Stratégie par défaut, cela signifie que code téléchargé à partir d’un intranet ou Internet ne fonctionne pas avec les wrappers.

Au lieu de donner des droits de code non managé à toutes les applications qui utilisent ces wrappers, il est préférable d’accorder ces droits uniquement au code wrapper. Si les fonctionnalités sous-jacentes n’exposent aucune ressource et que l’implémentation est également sécurisée, le wrapper doit simplement déclarer ses droits, ce qui permet à n’importe quel code d’effectuer un appel via celui-ci. Lorsque des ressources sont impliquées, le codage de sécurité doit être identique à l’exemple de code de bibliothèque décrit dans la section suivante. Comme le wrapper expose potentiellement les appelants à ces ressources, une vérification minutieuse de la sécurité du code natif est nécessaire. Celle-ci relève de la responsabilité du wrapper.

## <a name="library-code-that-exposes-protected-resources"></a>Code de bibliothèque exposant des ressources protégées.

L’approche suivante est la plus puissante et donc potentiellement dangereux (s’il est effectuée de manière incorrecte) pour le codage de sécurité : votre bibliothèque sert d’une interface pour un autre code accéder à certaines ressources qui ne sont pas autrement disponibles, tout comme les classes .NET Framework appliquent autorisations pour les ressources qu’ils utilisent. Chaque fois que vous exposez une ressource, votre code doit en premier lieu demander l’autorisation appropriée à la ressource (autrement dit, il doit effectuer une vérification de sécurité), puis généralement déclarer ses droits pour effectuer l’opération proprement dite.
...
|[Sécurité basée sur les rôles](role-based-security.md)|La sécurité basée sur les rôles .NET en détail et fournit des instructions pour l’utiliser dans votre code.|
