...
-   [<span data-ttu-id="97bdf-150">Les pauses du garbage collection sont trop longues</span><span class="sxs-lookup"><span data-stu-id="97bdf-150">Garbage collection pauses are too long</span></span>](#Issue_LongPauses)  
...
|<span data-ttu-id="97bdf-182">Contrôle des performances</span><span class="sxs-lookup"><span data-stu-id="97bdf-182">Performance checks</span></span>|  
|------------------------|  
...
-   <span data-ttu-id="97bdf-198">Le chargement et le déchargement fréquents de nombreux petits assemblys.</span><span class="sxs-lookup"><span data-stu-id="97bdf-198">Frequent loading and unloading of many small assemblies.</span></span>  
...
### <a name="workstation-or-server-garbage-collection"></a><span data-ttu-id="97bdf-242">Garbage collection pour station de travail et pour serveur</span><span class="sxs-lookup"><span data-stu-id="97bdf-242">Workstation or Server Garbage Collection</span></span>  
...
-   <span data-ttu-id="97bdf-259">Dans le débogueur WinDbg, après avoir chargé l’extension SOS, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-259">In WinDbg with the SOS debugger extension loaded, type the following command:</span></span>  
 ...
 [<span data-ttu-id="97bdf-265">Retour au début</span><span class="sxs-lookup"><span data-stu-id="97bdf-265">Back to top</span></span>](#top)  
...
-   [<span data-ttu-id="97bdf-279">Déterminez le nombre d'objets épinglés.</span><span class="sxs-lookup"><span data-stu-id="97bdf-279">Determine the number of pinned objects.</span></span>](#Pinned)  
...
3.  <span data-ttu-id="97bdf-304">Vous pouvez utiliser la commande suivante pour faire un dump des exceptions imbriquées.</span><span class="sxs-lookup"><span data-stu-id="97bdf-304">You can use the following command to dump nested exceptions.</span></span>  
...
-   <span data-ttu-id="97bdf-338">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-338">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-346">**!dumpheap -type System.String -min 150 -max 200**</span><span class="sxs-lookup"><span data-stu-id="97bdf-346">**!dumpheap -type System.String -min 150 -max 200**</span></span>  
...
1.  <span data-ttu-id="97bdf-367">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-367">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     <span data-ttu-id="97bdf-368">**!finalizequeue**</span><span class="sxs-lookup"><span data-stu-id="97bdf-368">**!finalizequeue**</span></span>  
...
-   <span data-ttu-id="97bdf-378">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-378">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     <span data-ttu-id="97bdf-380">Cette commande affiche la taille totale de tous les objets libres sur le tas managé, comme illustré dans l’exemple suivant.</span><span class="sxs-lookup"><span data-stu-id="97bdf-380">This command displays the total size of all the free objects on the managed heap, as shown in the following example.</span></span>  
...
-   <span data-ttu-id="97bdf-389">La commande suivante fait un dump pour l'espace libre dans la plage de la génération 0 :</span><span class="sxs-lookup"><span data-stu-id="97bdf-389">The following command dumps the free space within the generation 0 range:</span></span>  
...
-   <span data-ttu-id="97bdf-396">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-396">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     <span data-ttu-id="97bdf-397">**!gchandles**</span><span class="sxs-lookup"><span data-stu-id="97bdf-397">**!gchandles**</span></span>  
...
-   <span data-ttu-id="97bdf-414">Vous pouvez également déterminer la durée d’un garbage collection à l’aide des [événements ETW de garbage collection](../../../docs/framework/performance/garbage-collection-etw-events.md) en analysant les informations fournies.</span><span class="sxs-lookup"><span data-stu-id="97bdf-414">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../../docs/framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>  
  
     <span data-ttu-id="97bdf-415">Par exemple, les données suivantes indiquent une séquence d’événements qui se sont produits pendant un garbage collection non simultané.</span><span class="sxs-lookup"><span data-stu-id="97bdf-415">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-416">La suspension des threads managés a pris 26 us (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`).</span><span class="sxs-lookup"><span data-stu-id="97bdf-416">Suspending the managed thread took 26us (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`).</span></span>  
  
     <span data-ttu-id="97bdf-417">Le garbage collection réel a pris 4,8 ms (`GCEnd_V1` – `GCStart_V1`).</span><span class="sxs-lookup"><span data-stu-id="97bdf-417">The actual garbage collection took 4.8ms (`GCEnd_V1` – `GCStart_V1`).</span></span>  
  
     <span data-ttu-id="97bdf-418">La reprise des threads managés a pris 21 us (`GCRestartEEEnd` – `GCRestartEEBegin`).</span><span class="sxs-lookup"><span data-stu-id="97bdf-418">Resuming the managed threads took 21us (`GCRestartEEEnd` – `GCRestartEEBegin`).</span></span>  
  
     <span data-ttu-id="97bdf-419">La sortie suivante fournit un exemple de garbage collection d'arrière-plan et inclut les champs de processus, de thread et d'événement</span><span class="sxs-lookup"><span data-stu-id="97bdf-419">The following output provides an example for background garbage collection, and includes the process, thread, and event fields.</span></span> <span data-ttu-id="97bdf-420">(toutes les données ne sont pas affichées).</span><span class="sxs-lookup"><span data-stu-id="97bdf-420">(Not all data is shown.)</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-421">L'événement `GCStart_V1` à la ligne 42504816 indique qu'il s'agit d'un garbage collection d'arrière-plan, car le dernier champ est `1`.</span><span class="sxs-lookup"><span data-stu-id="97bdf-421">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="97bdf-422">Cela devient le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="97bdf-422">This becomes garbage collection No.</span></span> <span data-ttu-id="97bdf-423">102019.</span><span class="sxs-lookup"><span data-stu-id="97bdf-423">102019.</span></span>  
  
     <span data-ttu-id="97bdf-424">L'événement `GCStart` se produit, car un garbage collection éphémère doit être effectué avant de démarrer un garbage collection d'arrière-plan.</span><span class="sxs-lookup"><span data-stu-id="97bdf-424">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="97bdf-425">Cela devient le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="97bdf-425">This becomes garbage collection No.</span></span> <span data-ttu-id="97bdf-426">102020.</span><span class="sxs-lookup"><span data-stu-id="97bdf-426">102020.</span></span>  
  
     <span data-ttu-id="97bdf-427">À la ligne 42514170, le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="97bdf-427">At 42514170, garbage collection No.</span></span> <span data-ttu-id="97bdf-428">102020 prend fin.</span><span class="sxs-lookup"><span data-stu-id="97bdf-428">102020 finishes.</span></span> <span data-ttu-id="97bdf-429">Les threads managés sont redémarrés à cet endroit.</span><span class="sxs-lookup"><span data-stu-id="97bdf-429">The managed threads are restarted at this point.</span></span> <span data-ttu-id="97bdf-430">L'opération se termine au thread 4372, qui a déclenché ce garbage collection d'arrière-plan.</span><span class="sxs-lookup"><span data-stu-id="97bdf-430">This is completed on thread 4372, which triggered this background garbage collection.</span></span>  
  
     <span data-ttu-id="97bdf-431">Au thread 4744, une suspension se produit.</span><span class="sxs-lookup"><span data-stu-id="97bdf-431">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="97bdf-432">C'est la seule fois où le garbage collection d'arrière-plan doit suspendre les threads managés.</span><span class="sxs-lookup"><span data-stu-id="97bdf-432">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="97bdf-433">Cette durée est d'environ 99 ms ((63784407-63685394)/1000).</span><span class="sxs-lookup"><span data-stu-id="97bdf-433">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>  
  
     <span data-ttu-id="97bdf-434">L'événement `GCEnd` pour le garbage collection d'arrière-plan se trouve à la ligne 89931423.</span><span class="sxs-lookup"><span data-stu-id="97bdf-434">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="97bdf-435">Cela signifie que le garbage collection d'arrière-plan a duré environ 47 secondes ((89931423-42504816)/1000).</span><span class="sxs-lookup"><span data-stu-id="97bdf-435">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>  
  
     <span data-ttu-id="97bdf-436">Pendant que les threads managés sont exécutés, vous pouvez voir tous les garbage collection éphémères en cours.</span><span class="sxs-lookup"><span data-stu-id="97bdf-436">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>  
  
<a name="Triggered"></a>   
##### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="97bdf-437">Pour déterminer ce qui a déclenché un garbage collection</span><span class="sxs-lookup"><span data-stu-id="97bdf-437">To determine what triggered a garbage collection</span></span>  
  
-   <span data-ttu-id="97bdf-438">Dans le débogueur WinDbg ou Visual Studio avec l’extension de débogueur SOS chargée, tapez la commande suivante pour afficher tous les threads avec leurs piles d’appels :</span><span class="sxs-lookup"><span data-stu-id="97bdf-438">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>  
  
     <span data-ttu-id="97bdf-439">**~\*kb**</span><span class="sxs-lookup"><span data-stu-id="97bdf-439">**~\*kb**</span></span>  
  
     <span data-ttu-id="97bdf-440">Cette commande affiche une sortie similaire à la suivante :</span><span class="sxs-lookup"><span data-stu-id="97bdf-440">This command displays output similar to the following.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-441">Si le garbage collection a été provoqué par une notification de mémoire insuffisante du système d'exploitation, la pile des appels sera similaire, à ceci près que le thread sera le thread finaliseur.</span><span class="sxs-lookup"><span data-stu-id="97bdf-441">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="97bdf-442">Le thread finaliseur reçoit une notification asynchrone de mémoire insuffisante et déclenche le garbage collection.</span><span class="sxs-lookup"><span data-stu-id="97bdf-442">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>  
  
     <span data-ttu-id="97bdf-443">Si le garbage collection a été provoqué par l'allocation de mémoire, la pile se présente comme ceci :</span><span class="sxs-lookup"><span data-stu-id="97bdf-443">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-444">Un programme d'assistance juste-à-temps (`JIT_New*`) appelle `GCHeap::GarbageCollectGeneration`.</span><span class="sxs-lookup"><span data-stu-id="97bdf-444">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="97bdf-445">Si vous déterminez que les garbage collection de génération 2 sont dus à des allocations, vous devez déterminer quels objets sont collectés par les garbage collection de génération 2 et comment les éviter.</span><span class="sxs-lookup"><span data-stu-id="97bdf-445">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="97bdf-446">Autrement dit, vous devez déterminer la différence entre le début et la fin d'un garbage collection de génération 2, ainsi que les objets qui ont provoqué la collection de génération 2.</span><span class="sxs-lookup"><span data-stu-id="97bdf-446">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>  
  
     <span data-ttu-id="97bdf-447">Par exemple, tapez la commande suivante dans le débogueur pour afficher le début d’une collection de génération 2 :</span><span class="sxs-lookup"><span data-stu-id="97bdf-447">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>  
  
     <span data-ttu-id="97bdf-448">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="97bdf-448">**!dumpheap –stat**</span></span>  
  
     <span data-ttu-id="97bdf-449">Exemple de sortie (abrégé pour montrer les objets qui utilisent le plus d'espace) :</span><span class="sxs-lookup"><span data-stu-id="97bdf-449">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-450">Répétez la commande à la fin de la génération 2 :</span><span class="sxs-lookup"><span data-stu-id="97bdf-450">Repeat the command at the end of generation 2:</span></span>  
  
     <span data-ttu-id="97bdf-451">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="97bdf-451">**!dumpheap –stat**</span></span>  
  
     <span data-ttu-id="97bdf-452">Exemple de sortie (abrégé pour montrer les objets qui utilisent le plus d'espace) :</span><span class="sxs-lookup"><span data-stu-id="97bdf-452">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="97bdf-453">Les objets `double[]` ont disparu de la fin de la sortie, ce qui signifie qu'ils ont été collectés.</span><span class="sxs-lookup"><span data-stu-id="97bdf-453">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="97bdf-454">Ces objets représentent environ 70 Mo.</span><span class="sxs-lookup"><span data-stu-id="97bdf-454">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="97bdf-455">Les objets restants n'ont pas beaucoup changé.</span><span class="sxs-lookup"><span data-stu-id="97bdf-455">The remaining objects did not change much.</span></span> <span data-ttu-id="97bdf-456">Ces objets `double[]` étaient donc à l'origine du déclenchement du garbage collection de génération 2.</span><span class="sxs-lookup"><span data-stu-id="97bdf-456">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="97bdf-457">L'étape suivante consiste à déterminer pourquoi les objets `double[]` y sont situés et pourquoi ils sont morts.</span><span class="sxs-lookup"><span data-stu-id="97bdf-457">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="97bdf-458">Vous pouvez demander à un développeur d'où proviennent ces objets ou vous pouvez utiliser la commande **gcroot**.</span><span class="sxs-lookup"><span data-stu-id="97bdf-458">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>  
  
<a name="HighCPU"></a>   
##### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="97bdf-459">Pour déterminer si l’utilisation élevée du processeur est causée par le garbage collection</span><span class="sxs-lookup"><span data-stu-id="97bdf-459">To determine whether high CPU usage is caused by garbage collection</span></span>  
  
-   <span data-ttu-id="97bdf-460">Mettez en corrélation la valeur du compteur de performances de mémoire `% Time in GC` avec le temps d'exécution.</span><span class="sxs-lookup"><span data-stu-id="97bdf-460">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>  
  
     <span data-ttu-id="97bdf-461">Si la valeur `% Time in GC` connaît un pic en même temps que le temps d'exécution, le garbage collection provoque une utilisation élevée du processeur.</span><span class="sxs-lookup"><span data-stu-id="97bdf-461">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="97bdf-462">Dans le cas contraire, profilez l'application pour trouver où se produit l'utilisation élevée.</span><span class="sxs-lookup"><span data-stu-id="97bdf-462">Otherwise, profile the application to find where the high usage is occurring.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="97bdf-463">Voir aussi</span><span class="sxs-lookup"><span data-stu-id="97bdf-463">See also</span></span>

- [<span data-ttu-id="97bdf-464">Nettoyage de la mémoire</span><span class="sxs-lookup"><span data-stu-id="97bdf-464">Garbage Collection</span></span>](../../../docs/standard/garbage-collection/index.md)
