...
Vous avez également besoin de **Visual Studio 2017**, avec la charge de travail « Développement multiplateforme .NET Core » installée (voir la figure 5-2).
...
Avec Visual Studio Tools pour Docker, cette tâche se fait en quelques clics de souris seulement. Quand vous créez un projet dans Visual Studio 2017, vous voyez une option nommée **Activer la prise en charge de Docker** (voir la figure 5-3).
...
Vous pouvez aussi activer la prise en charge de Docker dans un projet d’application web .NET Core existant en cliquant avec le bouton droit sur le projet dans l’**Explorateur de solutions** et en sélectionnant **Ajouter** > **Prise en charge de Docker** (voir la figure 5-4).
...
Cette action ajoute un fichier *Dockerfile* au projet avec la configuration requise. Elle est disponible uniquement dans les projets d’application web .NET Core.

Pour ajouter un fichier *docker-compose.yml* pour la solution entière, cliquez avec le bouton droit sur le projet dans l’**Explorateur de solutions** et sélectionnez **Ajouter** > **Prise en charge de l’orchestrateur de conteneurs** (voir la figure 5-5).
...
**Figure 5-5**. Ajout de la prise en charge de l’orchestrateur de conteneurs à un projet existant dans Visual Studio 2017.

Dans les sections suivantes, nous décrivons les informations à ajouter dans chacun de ces fichiers. Visual Studio peut effectuer cette tâche à votre place, mais il est utile de savoir ce que contient un fichier Dockerfile.
...
Les images Docker sont créées automatiquement dans Visual Studio. Les étapes suivantes s’appliquent uniquement dans le cadre du workflow avec un éditeur ou une CLI. Elles sont expliquées pour vous permettre de bien en comprendre tous les dessous.
...
Pour créer une image personnalisée dans votre environnement local avec la CLI Docker et votre fichier Dockerfile, vous pouvez utiliser la commande docker build (voir la figure 5-5).
...
Quand vous utilisez Visual Studio pour créer un projet avec la prise en charge de Docker, vous ne créez pas une image explicitement. En fait, l’image est créée quand vous appuyez sur **F5** pour exécuter l’application ou le service Docker. Cette étape est automatique dans Visual Studio. Vous ne la verrez donc pas à l’écran, mais il est important d’en comprendre le mécanisme.
...
Ce fichier docker-compose.yml est une version simplifiée et fusionnée. Il contient des données de configuration statiques pour chaque conteneur (comme le nom de l’image personnalisée), qui s’appliquent toujours, ainsi que des informations de configuration variables selon l’environnement de déploiement, comme la chaîne de connexion. Dans des sections ultérieures, vous découvrirez comment vous pouvez fractionner le fichier de configuration docker-compose.yml en plusieurs fichiers docker-compose et substituer les valeurs en fonction de l’environnement et du type d’exécution (debug ou release).
...
L’ajout de la prise en charge de l’orchestrateur de conteneurs ajoute le fichier Dockerfile à votre projet (s’il n’existe pas déjà). Cette opération ajoute également des informations de configuration dans un fichier docker-compose.yml global au niveau de la solution. Vous voyez un nouveau nœud de projet (le fichier projet *docker-compose.dcproj*) dans l’**Explorateur de solutions** qui contient le fichier docker-compose.yml (voir la figure 5-8).
...
Vous pouvez ensuite ouvrir le fichier docker-compose.yml et le mettre à jour avec des fonctionnalités supplémentaires.

Vous pouvez aussi déployer une application multiconteneur avec un seul fichier docker-compose.yml en utilisant la commande `docker-compose up`.
...
### <a name="option-a-run-a-single-container-app"></a>Option A : Exécuter une application monoconteneur

#### <a name="docker-cli"></a>CLI Docker

Vous pouvez exécuter un conteneur Docker à l’aide de la commande docker run, comme dans la figure 5-9 ci-dessous :
...
Si vous n’avez pas ajouté la prise en charge de l’orchestrateur de conteneurs, vous pouvez également exécuter une application monoconteneur dans Visual Studio, en appuyant sur **F5**. Avec docker run, le conteneur s’exécute localement.

### <a name="option-b-run-a-multi-container-app"></a>Option B : Exécuter une application multiconteneur
...
Après l’exécution de la commande docker-compose up, l’application et ses conteneurs associés sont déployés sur votre hôte Docker.

#### <a name="visual-studio"></a>Visual Studio

L’exécution d’une application multiconteneur à l’aide de Visual Studio 2017 est simple. En plus d’exécuter l’application multiconteneur, vous pouvez déboguer tous ses conteneurs directement dans Visual Studio en définissant des points d’arrêt réguliers.
...
Avec [DC/OS](https://mesosphere.com/blog/2015/09/02/dcos-cli-command-line-tool-datacenter/) et [Kubernetes](https://kubernetes.io/docs/user-guide/deployments/), vous devez également utiliser des commandes et scripts de déploiement différents.
...
Cette URL dans le navigateur utilise le port 80 pour l’exemple de conteneur particulier présenté ici. Toutefois, en interne, les requêtes sont redirigées vers le port 5000, car le déploiement a été fait de cette façon avec la commande docker run, comme nous l’avons expliqué dans une étape précédente.
...
- **Steve Lasker. Vidéo .NET Docker Development with Visual Studio 2017**
