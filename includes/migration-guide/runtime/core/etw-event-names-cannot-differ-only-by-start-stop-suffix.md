...
|   |   |
|---|---|
|Détails|Dans .NET Framework 4.6 et 4.6.1, le runtime lève un <xref:System.ArgumentException> quand deux noms d’événements ETW (Event Tracing for Windows) diffèrent uniquement par le suffixe &quot;Start&quot; ou &quot;Stop&quot; (comme quand un événement est nommé <code>LogUser</code> et un autre <code>LogUserStart</code>). Dans ce cas, le runtime ne peut pas construire la source d’événements, qui ne peut pas émettre de journalisation.<ul><li>[X] quirked // Utilise un mécanisme pour activer ou désactiver la fonctionnalité, généralement à l’aide du ciblage de runtime, ou des fichiers AppContext ou config. Doit être activé automatiquement dans certains cas.</li><li>[ ] Build-time break // Provoque une rupture lors d’une tentative de recompilation</li></ul>|
